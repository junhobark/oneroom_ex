...
class MarketBoardWriteEdit extends StatefulWidget {
  final int id;

  const MarketBoardWriteEdit({Key? key, required this.id}) : super(key: key);

  @override
  State<MarketBoardWriteEdit> createState() => _MarketBoardWriteEditState();
}

class _MarketBoardWriteEditState extends State<MarketBoardWriteEdit> {
  //get
  ...
  Future<void> fetchData() async {
    var url =
    Uri.parse('http://10.0.2.2:8080/community/MARKET/${widget.id}/modify');
  ...
  void _marketBoardWriteEditState() async {
    final dio = Dio();
    var url = 'http://10.0.2.2:8080/community/MARKET/${widget.id}/modify';
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // 이전 화면인 MarketBoardScreen으로 돌아가며 새로운 글 데이터를 전달합니다
